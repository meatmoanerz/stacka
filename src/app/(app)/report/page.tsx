...
export default function ReportPage() {
  ...
  // Exclude CCM expenses to avoid double counting (they appear in the next period via invoice)
  const nonCcmExpenses = useMemo(() => expenses.filter(exp => !exp.is_ccm), [expenses])

  const totalIncome = incomeTotal?.total_income ?? 0
  const totalSpent = nonCcmExpenses.reduce((sum, exp) => sum + exp.amount, 0)
  ...
  return (
    <div className="p-4 space-y-6 pb-24">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
        <ReportCategoryRanking expenses={nonCcmExpenses} />
      </motion.div>

      {/* Budget vs Actual */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        <ReportBudgetVariance expenses={nonCcmExpenses} budget={budget ?? null} />
      </motion.div>

      {/* Household costs */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
        <ReportHouseholdCosts
          expenses={nonCcmExpenses}
          hasPartner={!!partner}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
        <ReportSavingsProgress expenses={nonCcmExpenses} selectedPeriod={selectedPeriod} />
      </motion.div>
